...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    $update_query = "UPDATE users SET fname = ?, lname = ?, phoneNumber = ?, address = ? WHERE id = ?";
    $update_stmt = mysqli_prepare($con, $update_query);
    mysqli_stmt_bind_param($update_stmt, "ssssi", $fname, $lname, $phoneNumber, $address, $user_id);
    
    if (mysqli_stmt_execute($update_stmt)) {
        // Refresh user data after update by re-executing the query
        mysqli_stmt_execute($stmt);
        $result = mysqli_stmt_get_result($stmt);
        $user = mysqli_fetch_assoc($result);
